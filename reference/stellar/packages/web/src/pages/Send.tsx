...
import { useSendStealth, getTokenList } from "@/hooks/useSendStealth";
...
export default function Send() {
  ...
  const isWalletAddress = isStellarAddress(recipient);
  const isMetaAddress = recipient.startsWith(META_ADDRESS_PREFIX);

  // Check if input looks like a wraith name
  ...
  const { metaAddress: lookedUpMeta } = useLookupMetaAddress(
    isWalletAddress ? recipient : undefined
  );
  ...
  const executeBatch = async () => {
    // Batch sending requires the sender contract to support batch operations
    // For MVP, we send individually
    setIsBatchPending(true);
    try {
      for (const entry of batchEntries) {
        await generateAndSend(entry.metaAddress, entry.amount, batchToken);
      }
      setIsBatchSuccess(true);
      toast(
        `Batch transfer complete -- ${batchEntries.length} recipients`,
        "success"
      );
    } catch (err) {
      toast(parseError(err), "error");
            ...
            {!stealthResult && (
              <div className="flex flex-col gap-8">
                ...
                <button
                  ...
                  className="w-full bg-primary text-primary-on-primary font-headline font-bold uppercase tracking-widest py-5 text-sm hover:brightness-110 transition-all active:scale-[0.99] disabled:opacity-30"
                >
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="text-[10px] text-on-surface-variant font-body flex flex-wrap gap-x-2">
                      ...
                      {sendHash && (
                        <span>
                          Tx:{" "}
                          <ExplorerLink href={txUrl(network, sendHash)}>
                            {sendHash.slice(0, 14)}...
                          </ExplorerLink>
                        </span>
                      )}
            ...
            {batchEntries.length > 0 && (
              <div className="space-y-2">
                ...
                {batchEntries.map((entry, i) => (
                  <div
                    key={i}
                    className="bg-surface-container-low p-4 flex items-center justify-between"
                  >
                    <div className="flex flex-col">
                      <span className="font-headline text-xs text-primary truncate max-w-[300px]">
                    ...
                    <button
                      ...
                      className="text-[10px] font-headline uppercase tracking-widest text-outline hover:text-error transition-colors"
                    >
                      Remove
                    </button>
                  </div>
                ...
                <button
                  ...
                  className="w-full bg-primary text-primary-on-primary font-headline font-bold uppercase tracking-widest py-5 text-sm hover:brightness-110 transition-all disabled:opacity-30 mt-4"
                >
                  {isBatchPending
                    ? "Sending..."
                    : `Send to ${batchEntries.length} recipient${
                        batchEntries.length > 1 ? "s" : ""
                      }`}
                </button>
              </div>
            )}
