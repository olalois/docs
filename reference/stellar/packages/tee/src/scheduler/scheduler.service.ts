...
export class SchedulerService {
  ...
  async executeScheduledPayments() {
    ...
    for (const sched of due) {
      try {
        ...
        const keypair = await this.tee.deriveAgentKeypair(sched.agentId);
        const stealthKeys = await this.tee.deriveAgentStealthKeys(sched.agentId);

        await this.tools.sendPayment(
          keypair, stealthKeys, agent.name, sched.recipient,
          sched.amount, sched.asset || 'XLM',
        );
        ...
        await this.db.pendingActions.save({
          ...
          message: `Scheduled payment executed: sent ${sched.amount} ${sched.asset || 'XLM'} to ${sched.recipient}. Next payment at ${new Date(nextRun * 1000).toLocaleString()}.`,
        });
        ...
        this.logger.log(`Scheduled: ${sched.amount} XLM to ${sched.recipient} for ${agent.name}`);
      } catch (err: any) {
        ...
        await this.db.pendingActions.save({
          agentId: sched.agentId,
          type: 'schedule_result',
          message: `Scheduled payment FAILED: could not send ${sched.amount} ${sched.asset || 'XLM'} to ${sched.recipient}. Error: ${err.message}`,
        });
  ...
  async backgroundScan() {
    ...
    const { scanAnnouncements } = await import('@wraith/sdk');
          ...
          if (!seenSet.has(m.stealthAddress)) {
            ...
            try {
              const res = await fetch(
                `https://horizon-testnet.stellar.org/accounts/${m.stealthAddress}`,
              );
              if (res.ok) {
            ...
            if (parseFloat(bal) > 0) {
              ...
              await this.db.pendingActions.save({
                ...
                message: `Received ${bal} XLM at stealth address ${m.stealthAddress.slice(0, 8)}...${m.stealthAddress.slice(-4)}. This payment is private and unlinkable.`,
              });
              ...
              this.logger.log(`New payment for ${agent.name}.wraith: ${bal} XLM`);
            }
          }
        }
  ...
  private async privacyAutoPilot(agentId: string, stealthCount: number) {
    ...
    if (settings?.autoWithdraw && settings?.preferredWithdrawAddress) {
      try {
        ...
        if (oldest && parseFloat(oldest.balance) > 1.5) {
          const result = await this.tools.withdraw(
            keypair, stealthKeys, oldest.address, settings.preferredWithdrawAddress,
          );
          ...
          if (!result.error) {
            await this.db.pendingActions.save({
              agentId,
              type: 'recommendation',
              message: `I autonomously withdrew ${result.withdrawn} XLM from your oldest stealth address to ${settings.preferredWithdrawAddress.slice(0, 8)}... for privacy maintenance. [tx](${result.txLink})`,
            });
