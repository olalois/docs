...
export class TeeController {
  ...
  @Get('attest/:agentId')
  @ApiOperation({ summary: 'Generate TEE attestation bound to agent public key' })
  async attest(@Param('agentId') agentId: string) {
    const keypair = await this.teeService.deriveAgentKeypair(agentId);
    const attestation = await this.teeService.getAttestation(keypair.publicKey());
    return {
