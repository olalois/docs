export function buildSystemPrompt(
  ...
  let prompt = `You are "${name}.wraith" — a privacy-obsessed AI agent living inside a Trusted Execution Environment on Stellar. You exist to protect your operator's financial privacy. You are not a generic assistant. You are a guardian.
...
YOUR CAPABILITIES:
- Send private XLM/USDC payments via stealth addresses (send_payment)
...
- Fund wallet from Friendbot (fund_wallet)
...
export const agentTools = [
  {
    functionDeclarations: [
      {
        name: 'send_payment',
        description: 'Send XLM or USDC privately to a .wraith name or meta-address via stealth address',
        parameters: {
          type: 'OBJECT',
          properties: {
            recipient: { type: 'STRING', description: 'Recipient .wraith name or st:xlm: meta-address' },
            amount: { type: 'STRING', description: 'Amount to send' },
            asset: { type: 'STRING', description: "Asset: 'XLM' (default) or 'USDC'" },
          },
      ...
      {
        name: 'create_invoice',
        description: 'Create a payment invoice with shareable link. Always include the markdownLink in your reply.',
        parameters: {
          type: 'OBJECT',
          properties: {
      ...
      },
      {
        ...
        parameters: {
          type: 'OBJECT',
          properties: { name: { type: 'STRING', description: 'Name to register (without .wraith)' } },
          required: ['name'],
        },
      },
      {
      ...
      {
        name: 'withdraw',
        description: 'Withdraw funds from a stealth address to a destination address',
        parameters: {
          type: 'OBJECT',
          properties: {
            from: { type: 'STRING', description: 'Stealth address (G...) to withdraw from' },
            to: { type: 'STRING', description: 'Destination address (G...)' },
          },
          required: ['from', 'to'],
        },
      },
      {
        name: 'withdraw_all',
        description: 'Withdraw from all detected stealth addresses. ALWAYS warn about privacy implications first.',
        parameters: {
          type: 'OBJECT',
          properties: { to: { type: 'STRING', description: 'Destination address (G...)' } },
          required: ['to'],
        },
      },
      {
        name: 'privacy_check',
        description: 'Deep privacy analysis — scores activity, detects patterns, recommends improvements',
        parameters: { type: 'OBJECT', properties: {}, required: [] },
      },
      {
        name: 'fund_wallet',
        description: 'Fund wallet with testnet XLM from Friendbot',
        parameters: { type: 'OBJECT', properties: {}, required: [] },
      },
      {
        name: 'pay_agent',
      ...
      },
      {
        ...
        parameters: {
          type: 'OBJECT',
          properties: {
            recipient: { type: 'STRING', description: 'Recipient .wraith name or meta-address' },
            amount: { type: 'STRING', description: 'Amount per payment' },
            interval: { type: 'STRING', description: "'hourly', 'daily', 'weekly', or 'monthly'" },
            end_date: { type: 'STRING', description: "Optional end date (e.g. '2026-05-01', 'in 30 days')" },
          },
      ...
      },
      {
        ...
        parameters: {
          type: 'OBJECT',
          properties: {
            schedule_id: { type: 'STRING', description: 'Schedule ID (first 8 chars are enough)' },
