...
const SLASH_COMMANDS: SlashCommand[] = [
  {
    command: "/send",
    description: "Send XLM privately via stealth address",
    params: [
      { key: "recipient", label: "Recipient", placeholder: "alice.wraith or st:xlm:..." },
      { key: "amount", label: "Amount (XLM)", placeholder: "1.0" },
    ],
    buildMessage: (v) => `Send ${v.amount} XLM to ${v.recipient}`,
  },
  {
    command: "/scan",
  ...
  },
  {
    ...
    params: [
      { key: "from", label: "From stealth address", placeholder: "G..." },
      { key: "to", label: "To destination", placeholder: "G..." },
    ],
  ...
  },
  {
    ...
    params: [
      { key: "to", label: "Destination address", placeholder: "G..." },
    ],
  ...
  },
  {
    ...
    params: [
      { key: "amount", label: "Amount (XLM)", placeholder: "100" },
      { key: "memo", label: "Memo", placeholder: "payment for services" },
    ],
    buildMessage: (v) => `Create an invoice for ${v.amount} XLM with memo "${v.memo}"`,
  },
  {
    command: "/invoices",
  ...
  },
  {
    ...
    params: [
      ...
      { key: "amount", label: "Amount (XLM)", placeholder: "5" },
    ],
    buildMessage: (v) => `Pay ${v.amount} XLM to ${v.name}.wraith`,
  },
  {
    command: "/resolve",
  ...
  {
    command: "/fund",
    description: "Fund your agent wallet",
    params: [
      { key: "method", label: "Method", placeholder: "Choose method", options: ["friendbot", "wallet"] },
    ],
    buildMessage: (v) => v.method === "friendbot"
      ? "Fund my wallet with testnet XLM from Friendbot"
      : "I want to fund my agent wallet from my personal wallet. Show me the agent's public key and instructions to send XLM.",
  },
  {
    command: "/info",
  ...
  },
  {
    ...
    params: [
      ...
      { key: "amount", label: "Amount", placeholder: "5" },
      { key: "interval", label: "Interval", placeholder: "daily, weekly, monthly" },
    ],
    buildMessage: (v) => `Schedule a recurring payment of ${v.amount} XLM to ${v.recipient} (${v.interval})`,
  },
  {
    command: "/schedules",
...
export default function Chat() {
  ...
  // Wizard state for interactive commands
  ...
  /* --- clear stale server url from localStorage --------------------- */
  useEffect(() => {
  ...
  /* --- on mount / wallet change: resolve agent --------------------- */
  useEffect(() => {
    let cancelled = false;
    ...
    async function init() {
      ...
      // Read agentId from localStorage directly to avoid dependency cycle
      const storedAgentId = localStorage.getItem(LS_AGENT_ID);

      if (!storedAgentId) {
        // No stored agent — look up by wallet
        try {
          ...
          // Server unreachable — show creation screen
        }
        if (!cancelled) setInitialLoading(false);
        return;
      }

      // We have a stored agentId — fetch the agent
      try {
        const res = await fetch(`${serverUrl}/agent/${storedAgentId}`);
        if (!res.ok) throw new Error("Agent not found");
        const data = await res.json();
        if (cancelled) return;

        const info: AgentInfo = {
          id: data.id ?? storedAgentId,
          name: data.name,
          publicKey: data.publicKey,
          metaAddress: data.metaAddress,
        };
        ...
        setIsConnected(true);
        // Fetch autonomous status
        await fetchAgentStatus(storedAgentId, info, cancelled);
      } catch {
        setAgentId(null);
        setAgentInfo(null);
  ...
  async function handleConnectWallet() {
    ...
    try {
      ...
      const addr = (result as any).address ?? result;
      ...
      const msg = e instanceof Error ? e.message : "Failed to connect. Make sure you have a Stellar wallet extension installed.";
      setWalletError(msg);
    } finally {
  ...
  async function handleCreate() {
    ...
    try {
      ...
      const signed = (signResult as any).signedMessage;
      if (!signed) {
      ...
      const res = await fetch(`${serverUrl}/agent/create`, {
        ...
        body: JSON.stringify({
          ...
          signature: typeof signed === "string" ? signed : Buffer.from(signed).toString("base64"),
          message,
        }),
      });
  ...
  async function sendToServer(text: string) {
    try {
      // Send previous messages as history (exclude current message — it's sent separately)
        ...
        <div className="w-full max-w-[480px] px-6">
          ...
          <div className="mb-12">
            ...
            <p className="text-on-surface-variant leading-relaxed">
              Sign in with your Stellar wallet to create or access your private AI agent.
            </p>
          </div>
          ...
          <p className="text-[10px] text-outline-variant mt-6 text-center font-mono">
            SECURED_VIA_PHALA_TEE — KEYS_NEVER_STORED
          </p>
        </div>
      </div>
    );
          ...
          <div className="mt-8 space-y-2 text-body-sm text-outline">
            ...
            <ul className="space-y-1.5">
              <li className="flex items-start gap-2">
                ...
                <span>Generate a Stellar wallet</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="text-on-surface-variant mt-0.5">&#8226;</span>
  ...
  return (
    <div className="flex h-screen w-screen bg-surface-container-lowest">
      ...
      {settingsOpen && (
        ...
        >
          <div
            className="bg-surface-container border border-outline-variant w-full max-w-md mx-4 p-6 space-y-5"
          ...
          >
            ...
            <div>
              ...
              <div className="space-y-1.5 text-xs text-outline">
                ...
                <p>Public Key: <span className="text-on-surface font-mono text-[10px] break-all">{agentInfo?.publicKey}</span></p>
                ...
                <p>Network: <span className="text-on-surface">Stellar Testnet</span></p>
              </div>
            </div>
            ...
            <div className="border-t border-outline-variant pt-4">
              ...
              {!exportedKey ? (
                <button
                  ...
                  className="text-xs py-2 px-4 border border-outline-variant text-on-surface-variant hover:text-on-surface hover:bg-surface-container-high transition-colors disabled:opacity-30"
                >
                  {exportLoading ? "..." : "Reveal Secret Key"}
                </button>
              ) : (
                <div className="space-y-2">
                  <div className="bg-surface-container-lowest border border-error/20 p-3">
      ...
      <div className="flex-1 flex flex-col">
        ...
        <header className="flex-shrink-0 h-14 bg-surface-container-lowest border-b border-outline-variant/10">
          <div className="flex items-center justify-between px-6 h-full">
            ...
            {/* Right: wallet + notifs + info toggle */}
                      ...
                      notifications.map((n) => (
                        ...
                        >
                          ...
                          <p className="text-[9px] text-outline-variant ml-3.5 mt-1">
                            {new Date(n.createdAt || (n.created_at || 0) * 1000).toLocaleString()}
                          </p>
                        </button>
                      ))
                    )}
        ...
        <footer className="flex-shrink-0 bg-surface relative">
                ...
                <div className="flex gap-2 px-4 pb-2">
                  {activeCommand.params[wizardStep].options!.map((opt) => (
                    <button
                      ...
                      className="px-4 py-1.5 text-xs font-bold uppercase tracking-wider border border-outline-variant text-on-surface-variant hover:bg-surface-container-high hover:text-on-surface transition-colors"
                    >
                      {opt}
                    </button>
                  ))}
          ...
          <div className={`flex items-center gap-3 px-6 py-4${activeCommand?.params[wizardStep]?.options ? " hidden" : ""}`}>
            ...
            <button
              ...
              className="bg-white text-surface px-5 py-2 font-label text-label-lg uppercase tracking-wider transition-colors hover:neon-glow disabled:opacity-40 disabled:cursor-not-allowed"
            >
              Send
            </button>
          </div>
        ...
        <aside className="hidden xl:flex w-[320px] flex-shrink-0 border-l border-outline-variant/10 bg-surface flex-col">
          ...
          <div className="px-6 py-4 border-b border-outline-variant/10">
            ...
            <p className="font-mono text-2xl font-medium text-on-surface">
              {parseFloat(agentBalance).toLocaleString(undefined, { maximumFractionDigits: 2 })}
              <span className="text-sm text-outline ml-2">XLM</span>
            </p>
          </div>

          <div className="p-6 space-y-4 flex-1 overflow-y-auto">
            <div>
              <span className="font-mono text-[10px] text-outline uppercase tracking-wider block mb-1">Public Key</span>
              <p className="font-mono text-xs text-on-surface-variant break-all bg-surface-container-low px-3 py-2 hover:bg-surface-bright transition-colors cursor-pointer"
                onClick={() => navigator.clipboard.writeText(agentInfo.publicKey)}
                title="Click to copy"
              >
                {agentInfo.publicKey}
              </p>
            ...
            <div>
              ...
              <p className="font-mono text-xs text-tertiary">Stellar Testnet</p>
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={() => setSelectedNotif(null)}>
          <div className="bg-surface-container border border-outline-variant/30 w-full max-w-md" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="px-5 pb-4">
              <p className="text-sm text-on-surface-variant whitespace-pre-wrap break-words">{selectedNotif.body}</p>
              <p className="text-[10px] text-outline-variant mt-3">
                {new Date(selectedNotif.createdAt || (selectedNotif.created_at || 0) * 1000).toLocaleString()}
              </p>
        ...
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4">
          <div className="bg-surface-container border border-outline-variant/30 w-full max-w-lg">
            ...
            <div className="flex items-center gap-3 px-6 pt-6 pb-4">
              ...
              <div>
                <h2 className="text-lg font-bold text-on-surface" style={{ fontFamily: "Space Grotesk, monospace" }}>
                  Welcome to Wraith
                </h2>
                <p className="text-xs text-outline">Your private AI agent on Stellar</p>
              </div>
            </div>
              ...
              {tourStep === 0 && (
                <div className="space-y-4 animate-fade-in">
                  <p className="text-sm text-on-surface-variant">
                    ...
                    It has its own Stellar wallet and stealth identity.
                  </p>
                ...
                <div className="space-y-4 animate-fade-in">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {[
                      ...
                      <div key={item.label} className="bg-surface-container-lowest border border-outline-variant/30 p-3">
              ...
              {tourStep === 2 && (
                <div className="space-y-4 animate-fade-in">
                  ...
                  <div className="space-y-3">
                    <div className="bg-surface-container-lowest border border-outline-variant/30 p-3">
                      <p className="text-xs text-on-surface font-mono mb-1">1. Fund your agent</p>
                      <p className="text-[10px] text-outline">Type <span className="text-on-surface-variant font-mono">/fund:friendbot</span> to get testnet XLM</p>
                    </div>
                    <div className="bg-surface-container-lowest border border-outline-variant/30 p-3">
                      <p className="text-xs text-on-surface font-mono mb-1">2. Send a payment</p>
                      <p className="text-[10px] text-outline">Type <span className="text-on-surface-variant font-mono">/send</span> or just say "send 5 XLM to alice.wraith"</p>
                    </div>
                    <div className="bg-surface-container-lowest border border-outline-variant/30 p-3">
                ...
                {[0, 1, 2].map((s) => (
                  <span
                    ...
                    className={`h-1.5 w-1.5 transition-colors ${
                      s === tourStep ? "bg-primary" : "bg-[#484848]"
                    }`}
                  />
                ))}
              </div>
    ...
    <div className="flex items-start gap-3 animate-fade-in">
      ...
      <div className="max-w-[85%] overflow-hidden prose-wraith">
        <ReactMarkdown
          components={{
            a: ({ href, children }) => (
              <a href={href} target="_blank" rel="noopener noreferrer" className="text-primary hover:text-primary-fixed underline">{children}</a>
            ),
          }}
        >{text}</ReactMarkdown>
      </div>
