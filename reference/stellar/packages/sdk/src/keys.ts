...
export function deriveStealthKeys(signature: Uint8Array): StealthKeys {
  ...
  const spendingInput = new Uint8Array(
    spendingPrefix.length + signature.length
  );
  spendingInput.set(spendingPrefix);
  spendingInput.set(signature, spendingPrefix.length);

  const viewingInput = new Uint8Array(
    viewingPrefix.length + signature.length
  );
  ...
  // Derive the clamped scalars (same as ed25519 internal key derivation)
