...
#[cfg(test)]
mod test {
    // Integration tests for the sender contract require a compiled announcer
    // WASM and a token contract. These are best run as integration tests in a
    // dedicated test harness. Unit-level smoke tests are omitted here to keep
    // the contract crate self-contained.
}
