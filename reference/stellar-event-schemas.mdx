...
**Forward-Compat Semantics**: Indexers and clients *must* gracefully ignore events with a `metadata_kind` they do not recognise. This allows new metadata formats to be deployed without breaking existing indexers.
...
You can query both simultaneously by including both contract IDs in your `getEvents` filter, or by running parallel queries for the different topic structures.
